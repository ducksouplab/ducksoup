...
export default ({ type }) => {
  const {
    dispatch,
    state: { started, flatFilters, groupedAudioFilters, groupedVideoFilters },
  } = useContext(Context);
  const groupedFilters =
    type === "audio" ? groupedAudioFilters : groupedVideoFilters;
  const [activeDisplay, setActiveDisplay] = useState("");

  useEffect(() => {
    if (flatFilters) {
      setActiveDisplay(
        flatFilters.filter(({ type: t }) => t === type)[0].display
      );
    }
  }, [flatFilters]);
    ...
    !started && (
      <>
        <label className="col-auto col-form-label">
          <span className="type">{type}</span> filters
        </label>
        <div className="col-auto">
          <select
            className="form-select"
            value={activeDisplay}
            onChange={(e) => setActiveDisplay(e.currentTarget.value)}
          >
